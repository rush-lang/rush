...
TEST_CASE( "rush::ast::constant_declaration", "[unit][ast]" ) {
	auto decl = decls::constant("abc", exprs::literal(1, ast::types::int_type));

	REQUIRE( decl->name() == "abc" );
	// REQUIRE( decl->type() == ast::types::int_type );
}

TEST_CASE( "rush::ast::variable_declaration", "[unit][ast]" ) {
	auto decl = decls::variable("def", exprs::literal(1, ast::types::int_type));
